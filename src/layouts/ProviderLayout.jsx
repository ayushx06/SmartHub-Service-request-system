...
export default function ProviderLayout() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-72 bg-indigo-950 text-white transition-transform duration-300 lg:translate-x-0 ${
          isSidebarOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="flex h-16 items-center justify-between border-b border-white/10 px-5">
          <div>
            <p className="text-lg font-bold">SmartHub</p>
            <p className="text-xs tracking-[0.22em] text-indigo-200">
              PROVIDER
            </p>
          </div>
          <button
            className="rounded-lg p-2 text-indigo-100 hover:bg-white/10 lg:hidden"
        ...
        <div className="absolute bottom-5 left-3 right-3">
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-lg border border-white/20 px-4 py-3 text-sm font-semibold text-indigo-50 hover:bg-white/10"
          >
